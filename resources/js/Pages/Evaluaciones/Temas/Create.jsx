...
export function TemaForm({ data, setData, errors, processing, areas, onSubmit, label }) {
    ...
    </CardContent></Card><div className="flex justify-end gap-3"><Button variant="outline" asChild><Link href={route('temas.index')}>Cancelar</Link></Button><Button disabled={processing} className="bg-brand-primary hover:bg-brand-primary/90"><Save className="h-4 w-4" />{label}</Button></div></form>;
}
